...
export default function TGAT2ResultPage() {
  ...
  if (questions.length === 0) {
    return (
      <div className="min-h-screen bg-[#050b14] flex flex-col items-center justify-center text-white">
        ...
        <Link href="/tgat2" className="text-neon-blue underline">Go take the exam</Link>
      </div>
    );
  }
  ...
  const history = getHistory("tgat2");
  ...
  // Time Analysis
  const totalTimeSpent = questions.length > 0 ? (examMode === 'exam' ? 4500 - timeRemaining : Object.values(perQuestionTime).reduce((a,b)=>a+b, 0)) : 0;
      ...
      <div className="max-w-5xl mx-auto space-y-6 relative z-10">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => router.push('/tgat2')} className="px-4 py-2 bg-neon-purple text-white font-bold rounded-xl text-sm flex items-center gap-2 hover:bg-purple-500 transition-all">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="md:col-span-1">
            <ScoreChart score={score} total={questions.length} label="คะแนน TGAT2" colorClass="from-neon-purple to-purple-500" />
          </div>
          ...
          <div className="md:col-span-2 grid grid-cols-2 gap-4">
            ...
            <div className="bg-slate-900/50 p-6 rounded-3xl border border-white/10 flex flex-col justify-center col-span-2">
              <div className="flex items-center gap-2 text-slate-400 mb-4">
                <TrendingUp className="w-5 h-5" /> พัฒนาการ (TGAT2)
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-slate-900/50 p-6 md:p-8 rounded-3xl border border-white/10 shadow-xl">
            <h3 className="text-lg font-bold text-white mb-6 flex items-center gap-2">
              <BarChart2 className="w-5 h-5 text-neon-purple" /> วิเคราะห์จุดอ่อนตามหัวข้อ
            </h3>
          ...
          <div className="bg-slate-900/50 p-6 md:p-8 rounded-3xl border border-white/10 shadow-xl">
            <h3 className="text-lg font-bold text-white mb-6 flex items-center gap-2">
              <BarChart2 className="w-5 h-5 text-pink-400" /> ความแม่นยำตามระดับความยาก
            </h3>
